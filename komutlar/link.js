...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`Reklam Filtresini Ayarlamak İçin \`+link aç\` | Kapatmak İstiyorsanız \`+link kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`Reklam Filtresini Ayarlamak İçin \`+link aç\` | Kapatmak İstiyorsanız \`+link kapat\` Yazabilirsiniz`)
